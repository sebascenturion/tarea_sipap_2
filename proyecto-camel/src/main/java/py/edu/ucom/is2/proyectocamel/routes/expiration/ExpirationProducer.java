...
public class ExpirationProducer extends RouteBuilder{
	...
	public void configure() throws Exception {
		from("timer:active-mq-timer?period=1000") //generamos un mensaje nulo cada 2 segundos
		.process(gen) //modificamos el mensaje con el ID generado
		.log("Mensaje enviado> ${body}") //logueamos el body del mensaje
		.to("activemq:expirationTest?explicitQosEnabled=true&timeToLive=500"); //encolamos el mensaje a la cola del Apache ActiqveMQ IS2
